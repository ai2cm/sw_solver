...
baseName = "./data/swes-numpy-0-M180-N90-T5-1-"
...
if what_to_plot == "h":
    ...
    if save_movie:
        ...
        with writer.saving(fig1, baseName + "h." + movie_format, Nt):
            ...
            for n in range(Nt):
                ...
                if n == 0:
                    ...
                    m1.drawcoastlines()
                    m1.drawparallels(np.arange(-80.0, 81.0, 20.0))
                    m1.drawmeridians(np.arange(0.0, 360.0, 20.0))
        ...
        for n in range(Nt):
            ...
            x1[1, :] = 0.0
            x1[-2, :] = 360.0
            color_scale = np.linspace(
                h[:, :, :].min(), h[:, :, :].max(), 30, endpoint=True
            )
        ...
        with writer.saving(fig2, baseName + "u." + movie_format, Nt):
            ...
            for n in range(Nt):
                ...
                if n == 0:
                    ...
                    m2.drawcoastlines()
                    m2.drawparallels(np.arange(-80.0, 81.0, 20.0))
                    m2.drawmeridians(np.arange(0.0, 360.0, 20.0))
                ...
                # Comment the following line to hide the title
                plt.title(
                    "Longitudinal velocity [m/s]: time = %5.2f hours\n"
                    % (t[n] / 3600.0)
                )
        ...
        with writer.saving(fig3, baseName + "v." + movie_format, Nt):
            ...
            for n in range(Nt):
                ...
                if n == 0:
                    ...
                    m3.drawcoastlines()
                    m3.drawparallels(np.arange(-80.0, 81.0, 20.0))
                    m3.drawmeridians(np.arange(0.0, 360.0, 20.0))
                ...
                # Comment the following line to hide the title
                plt.title(
                    "Latitudinal velocity [m/s]: time = %5.2f hours\n" % (t[n] / 3600.0)
                )
        ...
        with writer.saving(fig4, baseName + "quiver." + movie_format, Nt):
            ...
            for n in range(Nt):
                ...
                if n == 0:
                    ...
                    m4.drawcoastlines()
                    m4.drawparallels(np.arange(-80.0, 81.0, 20.0))
                    m4.drawmeridians(np.arange(0.0, 360.0, 20.0))
                    m4.drawmapboundary(fill_color="white")

                    Q = m4.quiver(
                        x4[1:-1:10, 0:-1:10],
                        y4[1:-1:10, 0:-1:10],
                        u[1:-1:10, 0:-1:10, 0].transpose(),
                        v[1:-1:10, 0:-1:10, 0].transpose(),
                        color="blue",
                    )

                else:
                    Q.set_UVC(
                        u[1:-1:10, 0:-1:10, n].transpose(),
                        v[1:-1:10, 0:-1:10, n].transpose(),
                    )

                # Comment the following line to hide the title
                plt.title(
                    "Velocity field [m/s]: time = %5.2f hours\n" % (t[n] / 3600.0)
                )
...
if what_to_plot == "vorticity":
    ...
    vorticity[:, 0, :] = (
        -(-1.5 * u[:, 0, :] + 2.0 * u[:, 1, :] - 0.5 * u[:, 2, :]) / dtheta
    )
    vorticity[:, -1, :] = (
        -(1.5 * u[:, -1, :] - 2.0 * u[:, -2, :] + 0.5 * u[:, -3, :]) / dtheta
    )
        ...
        with writer.saving(fig5, baseName + "vorticity." + movie_format, Nt):
            ...
            for n in range(Nt):
                ...
                if n == 0:
                    ...
                    m5.drawcoastlines()
                    m5.drawparallels(np.arange(-80.0, 81.0, 20.0))
                    m5.drawmeridians(np.arange(0.0, 360.0, 20.0))
                ...
                color_scale = np.linspace(
                    vorticity.min(), vorticity.max(), 30, endpoint=True
                )
                surf = m5.contourf(
                    x5[1:-1, :], y5[1:-1, :], vorticity[:, :, n], color_scale
                )

                # Comment the following line to hide the title
                plt.title(
                    "Relative vorticity magnitude [m/s]: time = %5.2f hours\n"
                    % (t[n] / 3600.0)
                )
...
if what_to_plot == "mesh":
    ...
    if projection == "cyl":
        m = Basemap(
            projection="cyl", llcrnrlat=-90, urcrnrlat=90, llcrnrlon=-22, urcrnrlon=382
        )
    elif projection == "ortho":
        m = Basemap(
            projection="ortho",
        ...
        m = Basemap(projection="npstere", boundinglat=45, lon_0=8.9511, resolution="l")

    phi1D = np.linspace(-20.0, 380.0, 21)
    ...
    m.drawmeridians(np.arange(-20.0, 380.0, 20.0))
